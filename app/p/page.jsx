...
export default function Page({ searchParams }) {
  ...
  if (!data && !err) {
    return <main style={{padding:24,fontFamily:"system-ui"}}>Henter profil…</main>;
  }

  if (err) {
    return <main style={{padding:24,fontFamily:"system-ui"}}>
      <h1>Kunne ikke hente data</h1>
      <p>{err}</p>
    </main>;
  }
  ...
  if (!p) {
    return <main style={{padding:24,fontFamily:"system-ui"}}>
      ...
      <p>Ingen light-profil for ID <b>{id}</b>. Redigér <code>app/p/data.json</code> i GitHub for at tilføje den.</p>
    </main>;
  }
...
>>>>>>> e9108ce (Disable ESLint in build; force-dynamic /p to avoid prerender error)
}
